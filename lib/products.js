...
function buscarPorId(id) {
  const [idBase, idVariacao] = String(id).split("::");
  const produto = produtos.find((p) => p.id === idBase);
  if (!produto) return null;
  if (!idVariacao || !Array.isArray(produto.variacoes)) return produto;

  const variacao = produto.variacoes.find((v) => v.id === idVariacao);
