...
function listar() {
  return produtos.filter((p) => !p.orderBump);
}

function buscarPorSlug(slug) {
  return produtos.find((p) => p.slug === slug && !p.orderBump) || null;
}
...
function buscarRelacionados(produto) {
  ...
  const complementares = produtos.filter(
    (item) => !item.orderBump && item.id !== produto.id && !idsIncluidos.has(item.id)
  );

  return [...relacionadosConfigurados, ...complementares];
}

function buscarOrderBump() {
  return produtos.filter((p) => p.orderBump && p.estoque > 0);
}

module.exports = { listar, buscarPorSlug, buscarPorId, buscarRelacionados, buscarOrderBump };
