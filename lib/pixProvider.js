...
const { TiggerPayPixProvider } = require("./tiggerpayPixProvider");
...
function getPixProvider(providerName = process.env.PIX_PROVIDER || "mock") {
  const tipo = providerName.trim().toLowerCase();
  switch (tipo) {
    case "tiggerpay":
      return new TiggerPayPixProvider();
    case "zuckpay":
      return new ZuckPayPixProvider();
    case "mock":
      return new MockPixProvider();
    default:
      throw new Error("PIX_PROVIDER invalido.");
  }
}
