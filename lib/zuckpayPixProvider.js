...
function webhookUrl() {
  // ZuckPay confirms payment directly to Dracofy, independently of local orders.
  return process.env.DRACOFY_WEBHOOK_URL?.trim() ||
...
class ZuckPayPixProvider {
  async createCharge({ orderId, valor, descricao, cliente, clickId, attribution }) {
    const body = {
      nome: cliente?.nome,
    ...
    // ZuckPay accepts tracking at the top level, not inside tracking_params.
    // Reference: ZuckPay/zuckpay-mcp src/tools/pix.ts and src/utils/schemas.ts.
    const tracking = require("./attribution").normalizeAttribution(attribution, clickId);
    for (const field of ["click_id", "fbclid", "fbc", "fbp", "utm_source", "utm_medium", "utm_campaign", "utm_content", "utm_term"]) {
      if (tracking[field]) body[field] = tracking[field];
    }
