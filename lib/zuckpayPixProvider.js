...
class ZuckPayPixProvider {
  async createCharge({ orderId, valor, descricao, cliente, clickId, attribution }) {
    ...
    // Campo não confirmado na documentação da ZuckPay: enviamos por via das
    // dúvidas (custo zero se for ignorado), mas a confirmação do Purchase
    // para a Dracofy NÃO depende disso — ver lib/dracofy.js.
    const tracking = require("./attribution").normalizeAttribution(attribution, clickId);
    if (Object.keys(tracking).length) body.tracking_params = tracking;
